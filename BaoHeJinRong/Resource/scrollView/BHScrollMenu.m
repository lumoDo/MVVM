...
@interface BHScrollMenu () <UIScrollViewDelegate> {
...
- (void)setup {
    ...
    _indicatorView = [BHIndicatorView initIndicatorView];
...
- (UIButton *)getButtonWithMenu:(BHMenu *)menu {
//    CGSize buttonSize = [menu.title sizeWithFont:menu.titleFont constrainedToSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(self.bounds) - 10) lineBreakMode:NSLineBreakByCharWrapping];
    CGRect buttonSize = [menu.title boundingRectWithSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(self.bounds) - 10) options:NSStringDrawingUsesLineFragmentOrigin attributes:[NSDictionary dictionaryWithObject:menu.titleFont forKey:NSFontAttributeName] context:nil];
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width / self.menus.count , buttonSize.size.height)];
